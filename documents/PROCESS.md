...
- Claude Code 裡實際跑了 `@` 選 resource 和 `/mcp__orderhub__low_stock_report` 這兩步（`/mcp` reconnect 後 `ToolSearch` 就看到 `mcp__orderhub__*` 五個工具冒出來）：
  - `@orderhub:orderhub://discount-rules` 選進來後問「Gold 會員買 1000 元商品應付多少」，agent 直接照 resource 內容答「900（1000 × 9折）」，沒有去讀 `OrderService.cs`。
  - `/mcp__orderhub__low_stock_report` 展開成範本裡那段「請用 low_stock 工具（threshold=10）查出低庫存商品，再用其他工具了解這些商品的近期訂單狀況，最後輸出採購建議表」，agent 照著呼叫了 `low_stock` 拿到 5 個商品——但範本第二句「再用其他工具了解這些商品的近期訂單狀況」卡住了：現有 5 個工具（`low_stock`／`get_order`／`customer_orders`／`create_order`／`cancel_order`）裡沒有一個能「依商品查訂單」，`get_order` 要訂單 Id、`customer_orders` 要客戶 Id，都不是「依 SKU 查近期銷量」。等於 prompt 範本寫的步驟，現有工具箱其實兜不起來，只能先用純庫存量拍一個粗估補貨量，銷量相關的「理由」欄位是空的。
  - 這是一個新發現：**prompt 範本和 tools 是分開維護的兩份東西，兩邊會各自漂移**——`low_stock_report` 的文字是照活動指南抄的，但當時的工具集本來就沒有「依商品查訂單」這支，範本從一開始就假設了一個不存在的能力。跟折扣規則那個「Resource 內容手動抄、跟 `OrderService` 脫鉉」是同一種坑，只是這次踩在 Prompt 身上：prompt 裡提到的工具名稱／步驟，要跟著 tools 一起被 review，不能寫完就不管。
- 想法（5c 第 3 點）：
  ...
  - Prompt 範本放 server vs 每個人自己打一段話：放 server 是**一次寫好、全隊透過同一個 slash command 共用**，範本措辞（例如「用 low_stock 工具查、再用其他工具了解訂單狀況、輸出表格含 SKU/建議補貨量/理由」）改一次全隊同步拿到新版；每個人自己打的話，問法、字眼、有沒有先查 low_stock 都不一樣，且沒有地方能沉澱「這樣問效果比較好」的經驗給下一個人複用。但這次實測也證明了放 server 有個反面代價：範本文字寫死了「再用其他工具了解訂單狀況」這種依賴工具能力的步驟，一旦工具集沒同步跟上，全隊共用的是同一份「過時指令」，比每個人各自亂打反而更難察覺——因為看起來是官方範本，容易被預設是對的。
