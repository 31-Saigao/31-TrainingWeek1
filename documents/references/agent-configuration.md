...
- 注意:前綴比對不是完整沙盒,串接指令(如 ;、&&)可能繞過,不應視為絕對安全防線
...
在 `training-repo/.claude/settings` 添加：
...
- `PostToolUse` 每次 agent 用 Edit / Write 改完檔案就自動 build，錯誤立即回饋給 agent（它會自己修）
...
- [ ] 請agent 創建一份 sample.txt, 查看 `/hooks` 文件夾裡面出現一個 `edit-log.txt` 文件
...
tools: Bash(dotnet test *), Read, Grep
---
...
**frontmatter 重點**：`name`、`description` 必填（description 決定 agent 何時會主動委派給它）；`tools` 是工具白名單（不填＝全部繼承）；可用 `model` 指定較便宜的模型跑機械性任務。
